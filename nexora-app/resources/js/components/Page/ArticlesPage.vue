...
const { loading, error, pageMeta, posts } = useBlogFeed('article');
</script>

<template>
    <main class="landing-main">
        <AsyncState :loading="loading" :error="error" loading-text="Загрузка статей…">
            <section class="landing-page-head">
                <div class="landing-container">
                    <nav class="landing-breadcrumb" aria-label="Хлебные крошки">
                        ...
                        <span class="landing-breadcrumb__current">Статьи</span>
                    </nav>

                    <h1>{{ pageMeta?.title || 'Статьи' }}</h1>
                    ...
                    <div v-else class="landing-blog-grid">
                        <article v-for="post in posts" :key="post.id" class="landing-blog-card">
                            <div class="landing-blog-card__thumb" aria-hidden="true">
                                ...
                                >
                                <span v-else class="landing-blog-card__placeholder">A</span>
                            </div>
                            <div class="landing-blog-card__body">
                                ...
                                <RouterLink
                                    :to="{ name: 'articles.view', params: { slug: post.slug } }"
